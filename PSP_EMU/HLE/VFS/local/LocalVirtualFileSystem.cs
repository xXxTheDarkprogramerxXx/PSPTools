...
//	import static pspsharp.HLE.modules.IoFileMgrForUser.PSP_O_WRONLY;
	...
	public class LocalVirtualFileSystem : AbstractVirtualFileSystem
	{
		...
		public override int ioRename(string oldFileName, string newFileName)
		{
			...
			if (log.DebugEnabled)
			{
				log.debug(string.Format("ioRename: renaming file '{0}' to '{1}'", oldFileName, newFileName));
			}

			if (!oldFile.renameTo(newFile))
			{
				log.warn(string.Format("ioRename failed: '{0}' to '{1}'", oldFileName, newFileName));
				return IO_ERROR;
			}
		...
		public override int ioDevctl(string deviceName, int command, TPointer inputPointer, int inputLength, TPointer outputPointer, int outputLength)
		{
			...
			switch (command)
			{
				...
				case 0x02425818:
				{
					...
					{
						log.warn("sceIoDevctl 0x02425818 bad param address " + string.Format("0x{0:X8}", inputPointer) + " or size " + inputLength);
						result = IO_ERROR;
					}
					break;
				}
