...
//	import static pspsharp.HLE.kernel.types.SceKernelThreadInfo.PSP_WAIT_MBX;
	...
	public class MbxManager
	{
		...
		public virtual int checkMbxID(int uid)
		{
			...
			{
				log.warn(string.Format("checkMbxID unknown uid=0x{0:X}", uid));
		...
		public virtual int sceKernelCreateMbx(string name, int attr, TPointer option)
		{
			if (option.NotNull)
			{
				...
				log.warn(string.Format("sceKernelCreateMbx option at {0}: size={1:D}", option, optionSize));
			}

			SceKernelMbxInfo info = new SceKernelMbxInfo(name, attr);
			if (log.DebugEnabled)
			{
				log.debug(string.Format("sceKernelCreateMbx returning {0}", info));
			}
		...
		public virtual int sceKernelSendMbx(int uid, TPointer msgAddr)
		{
			...
			if (!info.hasMessage())
			{
				...
				if (thread != null)
				{
					if (log.DebugEnabled)
					{
						log.debug(string.Format("sceKernelSendMbx waking thread {0}", thread));
					}
		...
		private int hleKernelReceiveMbx(int uid, TPointer32 addrMsgAddr, TPointer32 timeoutAddr, bool doCallbacks, bool poll)
		{
			...
			if (!info.hasMessage())
			{
				if (!poll)
				{
					if (log.DebugEnabled)
					{
						log.debug(string.Format("hleKernelReceiveMbx - {0} (waiting)", info));
					}
			...
			else
			{
				...
				if (log.DebugEnabled)
				{
					log.debug(string.Format("hleKernelReceiveMbx - {0} fast check succeeded", info));
				}
