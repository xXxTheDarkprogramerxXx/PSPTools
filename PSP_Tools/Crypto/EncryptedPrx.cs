using CSPspEmu.Core.Components.Crypto;
using System;
...
namespace PSP_Tools.Crypto
{
    public unsafe class EncryptedPrx
    {
        ...
        private TagInfo GetTagInfo(uint checkTag)
        {
            ...
            if (result == null)
            {
                throw new Exception($"Can't find tag1 0x{checkTag:X}");
            }
            return result;
        }
        ...
        private TagInfo2 GetTagInfo2(uint checkTag)
        {
            ...
            if (result == null) throw new Exception($"Can't find tag2 0x{checkTag:X}");
            return result;
        }
        ...
        protected byte[] DecryptPrx1(byte[] pbInBytes, bool showInfo = false)
        {
            ...
            fixed (byte* pbOut = pbOutBytes)
            {
                ...
                if (ret != 0)
                    throw new Exception(StringExtensions.Sprintf("mangle#7 returned 0x%08X, ", ret));
                ...
                if (ret != 0)
                    throw new Exception(StringExtensions.Sprintf("mangle#1 returned 0x%08X", ret));
        ...
        protected byte[] DecryptPrx2(byte[] pbIn, bool showInfo = false)
        {
            ...
            fixed (byte* tmp3 = tmp3Bytes)
            {
                ...
                {
                    throw new Exception("error in Scramble#1, ");
                }
                ...
                {
                    throw new Exception("error in Scramble#2, ");
                }
                ...
                *(uint*)outbuf = 0x014C;
                ...
                if (_kirk.HleUtilsBufferCopyWithRange(outbuf, 3000000, outbuf, 3000000,
                    ...
                    Kirk.ResultEnum.Ok)
                {
                    throw new Exception("error in sceUtilsBufferCopyWithRange 0xB, ");
                }

                if (PointerUtils.Memcmp(outbuf, tmp3, 0x14) != 0)
                {
                    throw new Exception("WARNING (SHA-1 incorrect), ");
                }
                ...
                {
                    throw new Exception("error in Scramble#3, ");
                }
        ...
        public class TagInfo
        {
            ...
            {
                return StringExtensions.Sprintf("TAG_INFO(tag=0x%08X, key=%s, code=%02X, codeExtra=%02X)", Tag,
        ...
        public class TagInfo2
        {
            ...
            public override string ToString()
            {
                return StringExtensions.Sprintf("TAG_INFO2(tag=0x%08X, key=%s, code=%02X)", Tag,
